...
size_t FileSize;
	...
	// Get temp file name
	NetPDLSchemaFileName= tempnam(NULL, NULL);
	if (NetPDLSchemaFileName == NULL)
	{
		nbGetLastErrorEx(__FILE__, __FUNCTION__, __LINE__,
			"Fatal error: cannot open a temp file to store the new NetPDL file.",
			ErrBuf, ErrBufSize);
...
// b: we have to use binary files in order to avoid problems with \n and such (fseek() does not work well with them)
#if defined(WIN32) || defined(WIN64)
	NetPDLSchemaFileHandle= fopen(NetPDLSchemaFileName, "w+bT");
#else
	NetPDLSchemaFileHandle= fopen(NetPDLSchemaFileName, "w+b");
#endif
	...
	// Close the temp file
	if (NetPDLSchemaFileHandle)
	{
		fclose(NetPDLSchemaFileHandle);
		// Now, let's remove the temp file from disk
		remove(NetPDLSchemaFileName);
	}

	if (NetPDLSchemaFileName)
		free(NetPDLSchemaFileName);
