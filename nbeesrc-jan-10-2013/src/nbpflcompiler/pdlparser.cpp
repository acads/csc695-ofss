...
void PDLParser::ParseElement(_nbNetPDLElementBase *element)
{
	...
	_nbNetPDLElementUpdateLookupTable *NetPDLElement = (_nbNetPDLElementUpdateLookupTable*)element;  //necessario a visitare l'attributo "action" per identificare la lookup-table-ADD
	#endif
	...
	switch(element->Type)
	{
	case nbNETPDL_IDEL_FIELD:	//[mligios] field recognition here?  //ERROR_HERE?				
		{
...
#ifdef ENABLE_LOOKUP_OPT	
//		cout<< "First time?"<<endl;  // [mligios] temp debug
		if(m_notyvisit==true)
		{
			cout<< "yes!"<<endl;
			//if ADD -> Save info about: /* Proto name - Table name */
			if (NetPDLElement->Action == nbNETPDL_UPDATELOOKUPTABLE_ACTION_ADD)	
			{
				cout<< "ADD recognized!"<<endl;
				unsigned int counter=0;				
				for (multimap<string, string>::iterator it = protosessiontab.find(m_CurrProtoSym->Name);it != protosessiontab.end() && (*it).second != NetPDLElement->TableName && (*it).first == m_CurrProtoSym->Name;it++)
					counter++;
	
				if(counter==protosessiontab.count(m_CurrProtoSym->Name))
					protosessiontab.insert(pair<string, string>(m_CurrProtoSym->Name,NetPDLElement->TableName));
		
				cout<<"map count(parse): "<< m_CurrProtoSym->Name << " actually: " <<protosessiontab.count(m_CurrProtoSym->Name) << endl;
				for (multimap<string, string>::iterator it = protosessiontab.begin();it != protosessiontab.end();++it)
				{
					cout << "  [" << (*it).first << ", " << (*it).second << "]" << endl;
				}			
			}else
				cout<< "not an ADD!"<<endl;
		}else
//			cout<< "no!"<<endl;	//temp debug	
#endif
		break;
...
//***************************THIS field IS A CHILD? FOUND CONTAINER!!
  ...
  if(field->DependsOn!=NULL)		
  {
	...
	{
		//LAST-ADD-MLIGIOS-TEMP-CHECK-ERROR?
		if(insideLoop==0)		
		{
			IntMap::iterator tmp;
...
void PDLParser::FastVisit(GlobalSymbols &globalSymbols)		//FAST VISIT - not correct! must be a proto!
{
	m_GlobalInfo = &globalSymbols.GetGlobalInfo();
	SymbolProto &protoSymbol=*m_GlobalInfo->GetStartProtoSym();
	_nbNetPDLElementProto *protoElement = protoSymbol.Info;
...
#ifdef ENABLE_LOOKUP_OPT
multimap<string,string> PDLParser::gettable(){   
	return protosessiontab;
}
#endif
...
//[mligios]
	case nbNETPDL_IDEL_UPDATELOOKUPTABLE:				//[mligios] FILL STRUCT HERE! is this the ADDD?!!??! (FIELD->ACTION="add")
#ifdef ENABLE_LOOKUP_OPT
	//cout<< "First time?"<<endl;  // [mligios] temp debug
	if(m_notyvisit==true)
	{
	cout<< "yes!"<<endl;		//if ADD -> Save info about: /* Proto name - Table name */
		if (NetPDLElement->Action == nbNETPDL_UPDATELOOKUPTABLE_ACTION_ADD)	
		{
			cout << "ADD recognized!"<<endl;
			unsigned int counter=0;				
			...
			cout<<"map count(visit): "<< m_CurrProtoSym->Name << " actually: " <<protosessiontab.count(m_CurrProtoSym->Name) << endl;
			for (multimap<string, string>::iterator it = protosessiontab.begin();it != protosessiontab.end();++it)
			{
			cout << "  [" << (*it).first << ", " << (*it).second << "]" << endl;
			}	
...
void PDLParser::VisitSwitch(_nbNetPDLElementSwitch *switchElement)
{
	...
	VisitExpressionInt(switchElement->ExprTree);		//[mligios] Need 4 checkin used fields!!
