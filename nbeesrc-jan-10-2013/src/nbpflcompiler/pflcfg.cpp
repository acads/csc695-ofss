...
/*
		if(f==2 || f==1){
		if(!bb->Code.Empty())
		cout<<endl<<"INFO: "<<bb->Code.Front()->Kind<<endl;
			else
		cout<<endl<<"else: "<<endl;
*/
/*
  ...
  myconv << f;
  std::string fname=string("mir_code_x_fine_1_")+ myconv.str() + string(".asm");
		ofstream irx(fname.c_str());		
		CodeWriter cw(irx);
		cw.DumpCode(&codeList); //dumps the MIR code
		irx.close();
*/
		//}
...
/*
		if(f==2 || f==1){
cout<<endl<<"FINE INFO"<<f<<endl;
		string name;
  std::stringstream myconv;
  myconv << f;
  std::string fname=string("mir_code_x_fine_2_")+ myconv.str() + string(".asm");
		ofstream irx(fname.c_str());		
		CodeWriter cw(irx);
		cw.DumpCode(&codeList); //dumps the MIR code
		irx.close();
