...
#include <map>
	...
	//#define ENABLE_FIELD_DEBUG  //Debug Info about Field & Loop Optimization!  
#endif
...

{
...
#ifdef ENABLE_LOOKUP_OPT
		/* [mligios]*/ //temp
		cout << "START-FRONTEND1 (TEMP-CHECK)-should be empty" <<endl; 
		multimap<string,string> ptab = NetPDLParser.gettable();
		for (multimap<string, string>::iterator it=ptab.begin(); it != ptab.end();it++)
		{
			cout << "FRONTEND-" << (*it).first << " " << (*it).second << " " << ptab.count((*it).first) <<endl; 
		}
		cout << "END-FRONTEND" <<endl; 
#endif
	...
	ofstream my_ir("Debug_Protocol_List_of_fields.txt");
	my_ir << "*********************   FRONTEND -> PROTOCOL LIST (GlobalSymbols is filled!)" <<endl; 
	...
	my_ir.close();
		...
		/* [mligios]*/ //temp
/*
		cout << "START-FRONTEND2 (TEMP-CHECK)-should be full" <<endl; 
		ptab = NetPDLParser.gettable();
		for (multimap<string, string>::iterator it=ptab.begin(); it != ptab.end();it++)
		{
			cout << "FRONTEND-" << (*it).first << " " << (*it).second << " " << ptab.count((*it).first) <<endl; 
		}
...
//here i need to know which lookup-table is really useful and which not... MIR Code is going to be generated!!
//[mligios]ENABLE_LOOKUP_OPT  
...
//ALL MIR Code is already generated!!
//[mligios]ENABLE_LOOKUP_OPT  
...
//here i need to know which lookup-table is really useful and which not...?? for the copro INIT !!??
//[mligios]ENABLE_LOOKUP_OPT  
...
//*****************************************  Simple Reduce of CFG  ***********************************//
		...
		while(reass_changed)
		{
			...
			m_Profiling << endl << endl << "Reduce of CFG before any opt required: " << (TicksAfterOpt - TicksBeforeOpt - MeasureCost) << " ticks";
		...
		if(optimizationCycles)
		{
			...
			while(reass_changed)
			{
...
#endif
				{
					...
					reass_changed = reass.run(); //FIXME - Ivano: this optimization inverts the order of the instruction on the reg exp coprocessor
...
//	set<SymbolProto*> alreadyCompacted; 
	#ifdef ENABLE_FIELD_DEBUG	
		ofstream my_ir("Debug_Fields_and_Loop_Optimizations.txt");
...
/*			Ivano: i commented this code because the compiler is more efficient without it
			...
			#ifdef ENABLE_FIELD_DEBUG
				my_ir <<" Protocol(After Loop & Field Optimization): "<<(*p)->Name;
				my_ir <<" Number of fields: "<<m_GlobalSymbols.GetNumField((*p))<<endl;
				my_ir <<"****************************************************************************"<<endl;		
			#endif
		}
	...
	switch(expr->GetOperator())
	{
		case REGEXP_OP:
		{
                ...
                for(set<pair<string,string> >::iterator current = fields.begin(); current != fields.end(); current++)
                {
					...
					field->Used = true;
					#ifdef ENABLE_FIELD_DEBUG
						ofstream my_ir("Debug_Fields_Filter.txt");
						my_ir <<"Set Field Info (PFL) "<<field->Name<<endl;
					#endif
...
void NetPFLFrontEnd::ParseExtractField(FieldsList_t *fldList)
{
	...
	{	
		...
		if(fld->Name != "allfields")
		{
			...
			#ifdef ENABLE_FIELD_DEBUG
				ofstream my_ir("Debug_Fields_Filter.txt");
				my_ir <<"Set Field Info (PFL) "<<fld->Name<<endl;
			#endif
		...
		else
		{
			...
			for(uint32_t i = 0; i < numFields; i++)
			{
				...
				#ifdef ENABLE_FIELD_DEBUG
					ofstream my_ir("Debug_Fields_Filter.txt");
					my_ir <<"Set Field Info (PFL) "<<current->Name<<endl;
				#endif
...
/*   	  if((&(*dest))->isAccepting())
  ...
  if(stateFrom->GetInfo()!=NULL)
  {
	...
	for(map<string, SymbolLabel*>::iterator it = toStateMap.begin(); it != toStateMap.end(); it++)
	{
		...
		manageLookUpTables(current);				//[mligios] IMPORTANT!! ^^^^^^^^^^^^^^^^^^^^
	}      
      ...
      ++i)
  {
    ...
    m_MIRCodeGen->CommentStatement("EARLY BEFORE SECTION FOR PROTOCOL: " +protocol->Name+ " END"); //[mligios]
...
void NetPFLFrontEnd::manageLookUpTables(SymbolProto *current)        
{
	...
	set<string> tables =  m_GlobalSymbols.GetAssociationProtosTable(current);  //Returns the set of tables used by protocol [mligios]
                                    ...
                                    bool fieldExtraction, EncapLabel_t input_symbol)
{
  ...
  if(m_Lower)
  {
  	 ...
  	 // generate and emit the "before" code for the protocol we're reaching		[mligios]  IMPORTANT!!!
